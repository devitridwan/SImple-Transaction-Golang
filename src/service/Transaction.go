...
func (this *ServiceKanggo) OrderTransaction(context echo.Context, db *sql.DB) error {
	...
	if err := context.Bind(&req); err != nil {
		...
		if val, ok := this.Token[token]; ok {
			...
			var status bool
			var price int
			err := db.QueryRow("select true, price*? from tbl_product where qty >= ? and id = ?", req.Amount, req.Amount, req.ProductId).Scan(&status, &price)
			if err == nil && status {
				_, err := db.Query("insert into tbl_transaksi (order_id, user_id, product_id, amount, status) values(?,?,?,?,?) ", orderId,
			...
			if val == req.Email {
				if val1, ok := this.TotalPrice[req.OrderId]; ok {
					if val1.TotalPrice == req.TotPrice {
						_, err := db.Query("update tbl_transaksi set status = ? where order_id = ?", "paid", req.OrderId)
						if err != nil {
							return context.JSON(http.StatusInternalServerError, res)
						} else {
							_, err := db.Query("update tbl_product set qty = qty-? where id = ?", val1.Qty, val1.ProductId)
							if err != nil {
								return context.JSON(http.StatusInternalServerError, res)
							} else {
								res.Status = "success"
