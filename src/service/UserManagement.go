...
func (this *ServiceKanggo) Registrasi(context echo.Context, db *sql.DB) error {
	...
	if err := context.Bind(&req); err != nil {
		...
		_, err := db.Query("insert into tbl_user (name, email, password, status) values (?, ?, ?, ?)", req.Name, req.Email,
...
func (this *ServiceKanggo) Login(context echo.Context, db *sql.DB) error {
	...
	if err := context.Bind(&req); err != nil {
		...
		err := db.QueryRow("select email from tbl_user where email = ? and password = ?", req.Email,
